...
QHeaderView::section { background-color: #3c3c3c; padding: 5px; border: none; border-bottom: 1px solid #555; font-weight: bold; }
...
class PreviewDialog(QDialog):
    ...
    def load_content(self):
        ...
        if error:
            QMessageBox.critical(self, "Error", f"Could not load file content: {error}")
            self.reject()
            return
        ...
        if self.is_image_file and encoding == 'base64':
            ...
            self.content_widget = QLabel("Preview is not available for this file type.")
    ...
    def toggle_edit_mode(self):
        if self.content_widget and isinstance(self.content_widget, QTextEdit):
            self.content_widget.setReadOnly(False)
            self.edit_button.setEnabled(False)
            self.save_button.setEnabled(True)
            self.content_widget.setFocus()

    def save_content(self):
        if not (self.content_widget and isinstance(self.content_widget, QTextEdit)):
            return
        new_content = self.content_widget.toPlainText()
        content_bytes = new_content.encode('utf-8')
        success, error = self.api_client.update_file_content(self.filename, content_bytes)
        if success:
            ...
            QMessageBox.critical(self, "Save Failed", f"Could not save file: {error}")
...
class MainWindow(QWidget):
    ...
    def init_ui(self):
        ...
        self.filter_all_btn = QPushButton("All Files"); self.filter_py_btn = QPushButton("Python (.py)"); self.filter_jpg_btn = QPushButton("Images (.jpg)")
        ...
        self.file_table.setColumnCount(len(self.COLUMNS)); self.file_table.setHorizontalHeaderLabels(self.COLUMNS.keys())
        ...
        self.status_bar = QLabel("0 items"); self.status_bar.setObjectName("StatusBar"); self.status_bar.setAlignment(Qt.AlignmentFlag.AlignCenter)
    ...
    def apply_filter(self, filter_type):
        ...
        self.filter_all_btn.setChecked(filter_type == 'all'); self.filter_py_btn.setChecked(filter_type == 'py'); self.filter_jpg_btn.setChecked(filter_type == 'jpg')
        ...
        elif filter_type == 'jpg': self.populate_table([f for f in self.all_files_data if f['name'].lower().endswith(('.jpg', '.jpeg'))])

    def show_file_context_menu(self, pos):
        if not self.file_table.selectedItems(): return
        menu = QMenu(self)
        rename_action = menu.addAction("Rename"); download_action = menu.addAction("Download"); delete_action = menu.addAction("Delete")
    ...
    def populate_table(self, files_data):
        ...
        for row, file_info in enumerate(files_data):
            def format_utc_to_local(date_str):
                try:
                    if isinstance(date_str, str): return datetime.fromisoformat(date_str.replace('Z', '+00:00')).astimezone(None).strftime('%d %b %Y at %H:%M')
    ...
    def sync_folder(self):
        ...
        if folder_path:
            ...
            if success: QMessageBox.information(self, "Sync Complete", message); self.refresh_files()
    ...
    def open_preview(self, item):
        ...
        if not filename.lower().endswith(('.c', '.jpg', '.jpeg', '.py', '.png', '.txt')):
             QMessageBox.information(self, "Preview Unavailable", "Preview is only supported for specified text and image files.")
             return
